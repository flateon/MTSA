...
ALL_DATASET = (
    ...
    # ('./dataset/ETT/ETTm1.csv', {'lamda': 0.5501, 'frequency': 'm', 'period': 96}),
    # ('./dataset/ETT/ETTm2.csv', {'lamda': 0.7837, 'frequency': 'm', 'period': 96}),
    ('./dataset/ETT/ETTh1.csv', {'lamda': 0.5505, 'frequency': 'h', 'period': 24}),
    ('./dataset/ETT/ETTh2.csv', {'lamda': 0.7854, 'frequency': 'h', 'period': 24}),
)
...
if __name__ == '__main__':
    ...
    for dataset_path, kwargs in ALL_DATASET:
        ...
        args.model = 'FLinear'
        for weight, weight_name in ALL_WEIGHT:
            args.fl_weight = weight
            for pred_len in PRED_LEN:
                ...
                print(
                    f"| {dataset.name:8} | {weight_name:20} | {pred_len:3} | {mse:6.4g} | {mae:6.4g} | {mape:6.4g} | {smape:6.4g} | {mase:6.4g} | {end - start: 6.3g} |")
                results.append(
                    [dataset.name, args.model, weight_name, pred_len, mse, mae, mape, smape, mase, end - start])

    # Create a Pandas DataFrame from the results list
    results_df = pd.DataFrame(results,
                              columns=["dataset", "model", "weight_name", "pred_len", "mse", "mae", "mape", "smape",
                                       "mase", "time"])

    # Save the DataFrame to a CSV file
    results_df.to_csv("results/test_fl.csv", index=False)
