...
def get_args():
    ...
    parser.add_argument('--msas', type=str, default='MIMO', help='multi-step ahead strategy used in TsfKNN, options: '
                                                                 '[MIMO, recursive]')
    parser.add_argument('--knn', type=str, default='brute_force', help='knn method used in TsfKNN, options: '
                                                                       '[brute_force, lsh]')
    parser.add_argument('--num_bits', type=int, default=4, help='num of bits for lsh method used in TsfKNN')
    parser.add_argument('--num_hashes', type=int, default=8, help='num of hashes for lsh method used in TsfKNN')
        ...
        for model_name in ALL_MODEL:
            ...
            for transform_name in ALL_TRANSFORM:
                ...
                for distance in ALL_DISTANCE:
                    ...
                    print(
                        f"| {dataset.name} | {model_name} | {distance} | {mse:.4g} | {mae:.4g} | {mape:.4g} | {smape:.4g} | {mase:.4g} |")
                    results.append(
                        [dataset.name, model_name, transform_name.removesuffix('Transform'), distance, mse, mae, mape,
                         smape,
                         mase])

    # Create a Pandas DataFrame from the results list
    results_df = pd.DataFrame(results,
                              columns=["dataset", "model", "transform", "distance", "mse", "mae", "mape", "smape",
                                       "mase"])
